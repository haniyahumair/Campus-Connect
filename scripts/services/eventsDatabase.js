...
async function loadEventDetails() {
  ...
  if (user) {
    ...
    heartIcon.src = isSaved
      ? "/assets/Icons/Heart filled peach.svg"
      : "/assets/Icons/Heart outline peach.svg";
    console.log(
      `Details page wishlist loaded — event ${event.id} is ${
        isSaved ? "saved" : "not saved"
      }`
    );
  } else {
    console.log(
      "No user logged in — skipping wishlist pre-fill on details page"
    );
  }

  document.querySelector(".save-event").addEventListener("click", async () => {
    console.log(
      `Save button clicked — event ID: ${event.id}, user: ${
...
async function addToCart() {
  try {
    ...
    showModal("Failed to add event to Cart!", "Please try again!.", "error", {
      autoClose: 3000,
      onClose: () => {
